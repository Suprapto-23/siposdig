...
@section('title', 'Edit Warga Binaan - SIPOSDIG')
...
<div class="w-full max-w-3xl mx-auto space-y-6 pb-12 animate-fade-in-up">
    ...
    <div class="flex items-center gap-4 mb-4">
        ...
        <div>
            <h2 class="font-jakarta text-2xl font-black text-slate-800 tracking-tight">Edit Data Warga</h2>
            <p class="text-[13px] text-slate-500 font-medium mt-0.5">Perbarui identitas dan status layanan <span class="font-bold text-blue-600">{{ $warga->nama_lengkap }}</span>.</p>
        </div>
    </div>

    @if ($errors->any())
        <div class="p-4 bg-rose-50 border border-rose-200 rounded-2xl">
            <div class="flex items-center gap-2 mb-2 text-rose-600 font-bold"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> Pembaruan Gagal</div>
            <ul class="list-disc list-inside pl-5 text-sm text-rose-500 font-medium">@foreach ($errors->all() as $error) <li>{{ $error }}</li> @endforeach</ul>
        </div>
    @endif
    ...
    <div class="bg-white/70 backdrop-blur-2xl border border-white rounded-[2rem] p-2 shadow-[0_8px_30px_rgba(0,0,0,0.03)]">
        <div class="bg-white rounded-[1.75rem] p-6 sm:p-10 border border-slate-100/50">
            ...
            <form method="POST" action="{{ route('kader.warga.update', $warga->id) }}" class="space-y-8" onsubmit="document.getElementById('btnSubmit').disabled=true; document.getElementById('btnText').innerHTML='Menyimpan...';">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div class="relative">
                            <label class="block text-[11px] font-extrabold text-slate-500 mb-2 tracking-widest uppercase">Nama Lengkap <span class="text-rose-500">*</span></label>
                            <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $warga->nama_lengkap) }}" required class="block w-full px-4 py-3.5 bg-slate-50/80 border border-slate-200 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 rounded-2xl text-sm font-bold text-slate-800 transition-all outline-none">
                        </div>
                    </div>
                </div>
                        ...
                        <div class="relative">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <label class="cursor-pointer relative">
                                    <input type="radio" name="jenis_kelamin" value="L" class="peer sr-only" required {{ old('jenis_kelamin', $warga->jenis_kelamin) == 'L' ? 'checked' : '' }}>
                                    <div class="w-full text-center py-3.5 border border-slate-200 rounded-2xl peer-checked:bg-blue-50 peer-checked:border-blue-500 peer-checked:text-blue-700 text-slate-500 bg-slate-50/50 hover:bg-white transition-all shadow-sm"><span class="text-sm font-bold">👨 Laki-Laki</span></div>
                                </label>
                                <label class="cursor-pointer relative">
                                    <input type="radio" name="jenis_kelamin" value="P" class="peer sr-only" required {{ old('jenis_kelamin', $warga->jenis_kelamin) == 'P' ? 'checked' : '' }}>
                                    <div class="w-full text-center py-3.5 border border-slate-200 rounded-2xl peer-checked:bg-rose-50 peer-checked:border-rose-500 peer-checked:text-rose-700 text-slate-500 bg-slate-50/50 hover:bg-white transition-all shadow-sm"><span class="text-sm font-bold">👩 Perempuan</span></div>
                                </label>
                            </div>
                        </div>

                        <div class="relative md:col-span-2">
                            <label class="block text-[11px] font-extrabold text-slate-500 mb-2 tracking-widest uppercase">Kategori Posyandu <span class="text-rose-500">*</span></label>
                            <div class="relative">
                                <select name="kategori" required class="block w-full px-4 py-3.5 bg-slate-50/80 border border-slate-200 focus:bg-white focus:border-blue-500 rounded-2xl text-sm font-bold text-slate-800 transition-all outline-none appearance-none cursor-pointer">
                                    <option value="Balita" {{ old('kategori', $warga->kategori) == 'Balita' ? 'selected' : '' }}>Bayi & Balita (0 - 59 Bulan)</option>
                                    <option value="Remaja" {{ old('kategori', $warga->kategori) == 'Remaja' ? 'selected' : '' }}>Remaja (10 - 18 Tahun)</option>
                                    <option value="Lansia" {{ old('kategori', $warga->kategori) == 'Lansia' ? 'selected' : '' }}>Lansia (Di atas 60 Tahun)</option>
                                </select>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="relative md:col-span-2">
                            ...
                            <textarea name="alamat" required rows="2" class="block w-full px-4 py-3.5 bg-slate-50/80 border border-slate-200 focus:bg-white focus:border-blue-500 rounded-2xl text-sm font-bold text-slate-800 transition-all outline-none resize-none">{{ old('alamat', $warga->alamat) }}</textarea>
                        </div>
                        
                        <div class="relative">
                            <label class="block text-[11px] font-extrabold text-slate-500 mb-2 tracking-widest uppercase">Nomor WhatsApp Aktif</label>
                            <input type="text" name="no_hp" value="{{ old('no_hp', $warga->no_hp) }}" class="block w-full px-4 py-3.5 bg-slate-50/80 border border-slate-200 focus:bg-white focus:border-blue-500 rounded-2xl text-sm font-bold text-slate-800 transition-all outline-none">
                        </div>
                        ...
                        <div class="relative">
                            ...
                            <div class="relative">
                                <select name="status" required class="block w-full px-4 py-3.5 bg-slate-50/80 border border-slate-200 focus:bg-white focus:border-blue-500 rounded-2xl text-sm font-bold text-slate-800 transition-all outline-none appearance-none cursor-pointer">
                                    <option value="aktif" {{ old('status', $warga->status) == 'aktif' ? 'selected' : '' }}>Aktif (Siap Menerima Pelayanan)</option>
                                    <option value="pending" {{ old('status', $warga->status) == 'pending' ? 'selected' : '' }}>Pending (Menunggu Konfirmasi)</option>
                                    <option value="nonaktif" {{ old('status', $warga->status) == 'nonaktif' ? 'selected' : '' }}>Nonaktif (Meninggal/Pindah Keluar)</option>
                                </select>
                                <div class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none text-slate-400"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg></div>
                            </div>
                        </div>
...
<style>
    ...
    .flatpickr-day.selected { background: #3b82f6 !important; border-color: #3b82f6 !important; border-radius: 0.5rem !important; font-weight: bold !important; }
